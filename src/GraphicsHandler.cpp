...
//		}, {
	...
	// TODO: move to own samplerinit function in first VKInit
...
void GraphicsHandler::VKSubInitDescriptorLayoutsAndPool (uint32_t nummeshes){
	...
	VkDescriptorPoolCreateInfo descriptorpoolcreateinfo{
			...
			vulkaninfo.numswapchainimages * (2 * VULKAN_MAX_LIGHTS + 4 * nummeshes + 2 + 1 + 1 + 1 + 1 + 1 + 2) +
...
void GraphicsHandler::VKSubInitPipeline (
	...
	if (shaderstages & VK_SHADER_STAGE_COMPUTE_BIT){
		...
		vkCreateDescriptorSetLayout(vulkaninfo.logicaldevice, &descsetlayoutcreateinfos[0], nullptr,
		                            &pipelineinfo->scenedsl);
		vkCreateDescriptorSetLayout(vulkaninfo.logicaldevice, &descsetlayoutcreateinfos[1], nullptr,
		...
		vkCreatePipelineLayout(vulkaninfo.logicaldevice, &pipelinelayoutcreateinfo, nullptr,
...
//		pipelineinfo==GraphicsHandler::vulkaninfo.linegraphicspipeline?VK_POLYGON_MODE
...
//		issm?VK_CULL_MODE_BACK_BIT:(pipelineinfo==&GraphicsHandler::vulkaninfo.grassgraphicspipeline?VK_CULL_MODE_NONE:VK_CULL_MODE_BACK_BIT),
	...
	vkCreateGraphicsPipelines(vulkaninfo.logicaldevice, VK_NULL_HANDLE, 1, &pipelinecreateinfo, nullptr,
	                          &pipelineinfo->pipeline);
...
void GraphicsHandler::VKSubInitDepthBuffer (){
	...
	GraphicsHandler::VKHelperTransitionImageLayout(vulkaninfo.depthbuffer.image,
	                                               ...
	                                               VK_IMAGE_LAYOUT_UNDEFINED,
	                                               VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
//	vulkaninfo.ssrrbuffers=new TextureInfo[vulkaninfo.numswapchainimages];
//	for(uint8_t scii=0;scii<vulkaninfo.numswapchainimages;scii++){
//		VKHelperInitTexture(&vulkaninfo.ssrrbuffers[scii],
//							vulkaninfo.swapchainextent.width, vulkaninfo.swapchainextent.height,
//							VULKAN_SWAPCHAIN_IMAGE_FORMAT,
//							VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
...
//							genericsampler);
	// TODO: remove this
	// very janky workaround to check if general sci layout works
//		VKHelperTransitionImageLayout(vulkaninfo.swapchainimages[scii],
//									  1,
//									  VULKAN_SWAPCHAIN_IMAGE_FORMAT,
//									  VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
