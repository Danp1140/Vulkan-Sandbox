...
void Light::recalculateProjectionMatrix () {
	if (type == LIGHT_TYPE_SUN) {
		...
		for (uint8_t i = 0; i < 8; i++) {
			if (shadowtype == SHADOW_TYPE_CAMERA_SPACE_PERSPECTIVE ||
				shadowtype == SHADOW_TYPE_LIGHT_SPACE_PERSPECTIVE) {
...
void Light::recalculateViewMatrix () {
	if (shadowtype == SHADOW_TYPE_LIGHT_SPACE_PERSPECTIVE) {
		...
		lspviewnotranslate[3][0] = 0.;
		lspviewnotranslate[3][1] = 0.;
		lspviewnotranslate[3][2] = 0.;
...
void Light::recalculateLSPSMMatrix (glm::vec3 cameraforward, glm::vec3 camerapos, glm::vec3* b, uint8_t numb) {
	...
	if (shadowtype == SHADOW_TYPE_UNIFORM) lspsmmatrix = glm::mat4(1.);
	else if (shadowtype == SHADOW_TYPE_CAMERA_SPACE_PERSPECTIVE) {
		glm::vec3 min, max;
		...
		glm::vec3 P = camerapos, v = -cameraforward;
		float t = 0.;
		for (uint8_t x = 0; x < 3; x++) {
			if (v[x] < 0.) {
				if (t * v[x] + P[x] > min[x]) {
...
//		lspprojection = glm::frustum<float>(-1., 1., -1., 1., t, t - 0.5);
		lspprojection = glm::mat4(1.f);
		lspsmmatrix = GraphicsHandler::projectFromView(lspprojection,
													   camerapos + v * t,
...
void Light::updateMatrices (glm::vec3 cameraforward, glm::vec3 camerapos, glm::vec3* b, uint8_t numb) {
