...
void Light::recalculateProjectionMatrix(){
	...
	//the PLANE light will use glm::ortho, with a specific size
	//the SUN light can be assumed to be orthographic, as it's distance
	//from the scene is effectively infinite (although in practice we would like it to be moveable)
	//but the POINT light theoretically would use glm::perspective, but in a sphere...does that work????
	//like, fov=6.28???
...
//	projectionmatrix=glm::ortho<float>(-5, 5, -5, 5, 0, 100);
	...
	if(type==SUN) projectionmatrix=glm::ortho<float>(-25, 25, -25, 25, 0, 10000);
	else if(type==PLANE) projectionmatrix=glm::ortho<float>(-5, 5, -5, 5, 0, 100);
